import { BlockStack, Box, Button, Card, ColorPicker, Divider, Grid, InlineGrid, InlineStack, Layout, ProgressBar, Text, TextField, Thumbnail, Popover, Collapsible, hsbToHex } from '@shopify/polaris'
...
const SetupStep = () => {
    ...
    useEffect(() => {
        console.log("use Effect executed before:", colorString)
        
        // setColorString(hsbToHex(color));
        console.log("use Effect executed after:", colorString)
        // setColorString(hsbaToHex(color.hue, color.saturation, color.brightness, color.alpha))
    }, [])
    ...
    //         status: false,
    //         progressVal: 0,
    //     },  // Progress for the second card
    //     card3: {
    //         status: false,
    //         progressVal: 0,
    //     },  // Progress for the third card (or more cards as needed)
    // });
    ...
    // const hsbaToHex = (hue, saturation, brightness, alpha = 1) => {
    //     // Convert the hue, saturation, and brightness to RGB
    //     const h = hue / 360; // Normalize the hue to 0-1
    //     const s = saturation;
    //     const v = brightness;

    //     // When brightness is 0, it's always black
    //     if (v === 0) {
    //         return '#000000';
    //     }
    ...
    //     const m = v - c; // Matching value to add to RGB

    //     let r, g, b;

    //     // Determine RGB based on the hue
    //     if (h >= 0 && h < 1 / 6) {
    //         r = c;
    //         g = x;
    //         b = 0;
    ...
    const handleColorChange = (newColor) => {
        ...
        setColor(newColor);
        // const { hue, saturation, brightness, alpha } = newColor;
        // Convert HSBA to HEX before setting the state
        // setColorString(hsbToHex(color));
        // const value = hsbaToHex(hue, saturation, brightness, alpha)
            ...
            <BlockStack gap={1000}>
                <RecoraDiv style={{ maxWidth: "1024px", margin: "0 auto", width: "100%", }}>
                    <Layout>
                        ...
                        <Card>
                            <Box>
                                <img src="https://procdn.swymrelay.com/embed/assets/customizationPreviewBackgroundPdp.svg"/>
                            </Box>
                        </Card>
                    </Layout.Section >
                    <Layout.Section variant="oneHalf">
                        ...
                        <BlockStack gap={800} >
                            ...
                            <BlockStack gap={200}>
                                ...
                                <InlineGrid gap="400" columns={{ xs: 1, sm: 2, md: 2, lg: 2, xl: 2 }}>
                                    {iconPetterns.map((IconStyle , index) =>
                                        <Card key={index}>
                                            <InlineStack gap={400} blockAlign="center" wrap={false}>
                            ...
                            <BlockStack gap={200}>
                                <Text as="p" variant='bodyMd' fontWeight='bold'>Settings</Text>
                                <InlineGrid gap="400" columns={{ xs: 1, sm: 2, md: 2, lg: 2, xl: 2 }}>
                                    <Card>
                                        <BlockStack gap={100}>
                                            ...
                                            <Divider />
                                            <div className='view-color' width="50px" height="50px" style={{ background: `hsa(${color})` }}></div>
                                            <Text as="p">{colorString}</Text>
                                            <TextField
                                                label="Price"
                                                value={colorString}
                                                ...
                                                prefix={<span style={{
                                                    background: colorString,  // Background color from colorString
                                                    display: 'inline-block',
                                                    width: '20px',
                                                    height: '20px',
                                                    marginRight: '8px',  // Space between color box and text
                                                    borderRadius: '4px'  // Optional: to make the color box rounded
                                                }} />}
                                                autoComplete="off"
                                            />
                                            <BlockStack>
                                                <Box>
                                                    ...
                                                    className="tests"
                                                        ...
                                                        activator={<Button onClick={togglePopoverActive}>Change Color</Button>}  
                                                    ...
                                                    {/* <Collapsible
                                                            ...
                                                            expandOnPrint
                                                        > */}
                                                    {/* <ColorPopover popoverActive={true}> */}
...
const ColorPopover = ({ children, popoverActive }) => {
    return (
        <div
            className={`Recora-colorPopOver-wrapper ${popoverActive ? 'recora-animation-part' : ''}`}
            style={{
                paddingInline: "40px",
